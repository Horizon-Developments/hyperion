...
cbox:AddLabel({
  Text = "Downside, blocks in the same position are locally removed.",
  DoesWrap = true
})
...
Env["paint_aura"].fix_msg = function(msg)
  ...
  return msg
    ...
    .. (function()
      ...
      return l:sub(_1, _1)
        .. l:sub(_2, _2)
        .. (math.random() > 0.2 and l:sub(_3, _3) or "")
    end)()
end
...
local crasher_start = bhelper(function(c, d, e)
  ...
  if not SharedData.crasher_init
    or not SharedData.crasher_init.Brick
    or not SharedData.crasher_init.Brick.Parent then
...
local crasher_init = bhelper(function(c, d)
  ...
  local buildTool = localplr.Backpack:FindFirstChild("Build", true)
    or (localplr.Character and localplr.Character:FindFirstChild("Build", true))

  local event = buildTool and buildTool:FindFirstChild("Event", true)
  ...
  conn = folder.ChildAdded:Connect(function(brick)
    ...
    local sides = {
      {
        Enum.NormalId.Top,
        brick.Position + brick.CFrame.UpVector * brick.Size.Y / 2
      },
      {
        Enum.NormalId.Bottom,
        brick.Position - brick.CFrame.UpVector * brick.Size.Y / 2
      ...
      {
        Enum.NormalId.Front,
        brick.Position + brick.CFrame.LookVector * brick.Size.Z / 2
      },
      {
        Enum.NormalId.Back,
        brick.Position - brick.CFrame.LookVector * brick.Size.Z / 2
      ...
      {
        Enum.NormalId.Left,
        brick.Position - brick.CFrame.RightVector * brick.Size.X / 2
      },
      {
        Enum.NormalId.Right,
        brick.Position + brick.CFrame.RightVector * brick.Size.X / 2
